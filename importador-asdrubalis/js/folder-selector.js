...
document.addEventListener('DOMContentLoaded', function() {
    var textarea = document.getElementById('url-input');
    var countDisplay = document.getElementById('url-count');

    textarea.addEventListener('input', function() {
        var urls = textarea.value.trim().split(/\s+/); // Divide el texto por cualquier espacio en blanco
        var count = urls.filter(Boolean).length; // Filtra elementos vacíos para no contarlos
        countDisplay.textContent = count; // Actualiza el contador visual en el front
    });
});